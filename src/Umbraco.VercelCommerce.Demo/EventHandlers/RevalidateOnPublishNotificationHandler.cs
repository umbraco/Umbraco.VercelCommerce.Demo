...
namespace Umbraco.VercelCommerce.Demo.EventHandlers
{
    ...
    {
        private const string COLLECTION_CHANGE_EVENT = "collections/update";
        ...
        public async Task HandleAsync(ContentPublishedNotification notification, CancellationToken cancellationToken)
        {
            ...
            {
                if (item.ContentType.Alias.InvariantContains("collection") && !toRevalidate.Contains(COLLECTION_CHANGE_EVENT))
                {
                    toRevalidate.Add(COLLECTION_CHANGE_EVENT);
                }
                else if (item.ContentType.Alias.InvariantContains("product") && !toRevalidate.Contains(PRODUCT_CHANGE_EVENT))
                {
                    toRevalidate.Add(PRODUCT_CHANGE_EVENT);
                }
                else if (item.ContentType.Alias.InvariantContains("page") && !toRevalidate.Contains(PAGE_CHANGE_EVENT))
                {
